<?php
	function js_signin() {
		...
		$loginkey = md5($_POST['password']);
			...
			$_SESSION['mysitei_user'] = js_let_me_user($loginname, $loginkey);
			$_SESSION['mysitei_account'] = js_logged_account($loginname);
			header( "Location: index.php" );
			
		}   else {
			$errmsg_arr[] = 'Either your username or password is incorrect!';
	...
	function js_is_logged(){
		$myloginid = isset( $_SESSION['mysitei_user'] ) ? $_SESSION['mysitei_user'] : "";
...
function logout() {
  unset( $_SESSION['mysitei_user'] );
  unset( $_SESSION['mysitei_account'] );
  header( "Location: index.php" );
}
